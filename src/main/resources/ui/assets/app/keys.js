export async function init({ api, Auth, showAlert, setTitle, clearAlerts }) {
  ...
  function renderKeys(keys) {
    ...
    tbody.innerHTML = keys.map(k => {
      ...
      const curveClass = curveUpper === "ED25519" ? "bg-purple-lt" : "bg-indigo-lt";
  ...
  function actionsForKey(k, ctx = {}) {
    ...
    let allowEncrypt = (scopes.has("ENCRYPT") || scopes.has("DECRYPT")) && isSecp;
    let allowPublic = scopes.has("PUBLIC");

    if (status === "DESTROYED") {
      allowSigs = false; allowEncrypt = false; allowPublic = false;
  ...
  function openCrypto(key, mode = "encrypt") {
      ...
      try { opSelect.value = initialOp; } catch {}
