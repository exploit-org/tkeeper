export async function init({api, Auth, showAlert, clearAlerts}) {
    ...
    const isManual = (st.sealedBy || "").toLowerCase() === "shamir";
    ...
    if (!isManual) {
        sharesEl.closest(".mb-4").classList.add("d-none");
        resetEl.closest(".form-check").classList.add("d-none");
        statusEl.textContent = "Keeper is sealed.";
        ...
        submit.addEventListener("click", async () => {
            ...
            try {
                const res = await api.getJson("/v1/keeper/system/unseal");
