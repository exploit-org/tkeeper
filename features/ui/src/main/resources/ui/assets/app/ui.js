...
async function refreshSystemInfo() {
  ...
  if (!Auth.hasPermission?.("tkeeper.control.system")) return;

  try {
    SYSTEM_INFO = await api.getSystem();
  } catch {
    SYSTEM_INFO = null;
  }
}

function isMonoQuorum() {
  const threshold = Number(SYSTEM_INFO?.threshold ?? 0) || 0;
  const total = Number(SYSTEM_INFO?.totalPeers ?? 0) || 0;
  return threshold === 1 && total === 1;
}
...
function applyNavPermissions() {
  ...
  links.forEach((a) => {
    ...
    const hiddenByQuorum = a.getAttribute("data-hide-quorum") === "mono" && isMonoQuorum();
    const item = a.closest(".nav-item");

    if (item) {
      item.classList.toggle("d-none", hiddenByQuorum);
    }
